...
struct spacetime_dimension {
	bool operator==( const spacetime_dimension & ) const { return true; }
	bool operator<( const spacetime_dimension & ) const { return false; }
};
...
struct vector_space {
	...
	vector_space( algebraic_field f, std::variant<int, spacetime_dimension, clifford_algebra_target_dimension> d,
		vector_space_metric m
	)
